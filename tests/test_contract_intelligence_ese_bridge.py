...
def test_build_bid_review_ese_config_uses_domain_roles_and_dry_run_defaults() -> None:
    ...
    assert (
        ...
        in cfg["roles"]["document_intake_analyst"]["prompt"]
    )
...
def test_run_bid_review_with_ese_executes_through_pipeline(tmp_path: Path) -> None:
    ...
    state = json.loads(
        (artifacts_dir / "pipeline_state.json").read_text(encoding="utf-8")
    )
    ...
    first_role_output = json.loads(
        (artifacts_dir / "01_document_intake_analyst.json").read_text(encoding="utf-8")
    )
...
def test_build_bid_review_ese_config_supports_agency_perspective() -> None:
    ...
    cfg = build_bid_review_ese_config(
        project_dir=project_dir, analysis_perspective="agency"
    )

    assert cfg["input"]["analysis_perspective"] == "agency"
    assert "from the agency perspective" in cfg["input"]["scope"]
    assert (
        "agency-side construction bid review"
        in cfg["roles"]["contract_risk_analyst"]["prompt"]
    )


def test_build_bid_review_ese_config_prioritizes_salient_clauses_in_prompt(
    tmp_path: Path,
) -> None:
    project_dir = tmp_path / "ese-salience"
    project_dir.mkdir()
    prime_lines = [
        f"Section {index}.0 Administrative Filler\nRoutine coordination language only."
