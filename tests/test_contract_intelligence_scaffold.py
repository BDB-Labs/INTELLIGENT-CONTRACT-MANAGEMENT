...
document_classifier = importlib.import_module(
...
role_catalog = importlib.import_module(
...
def test_bid_review_pipeline_has_expected_stage_order() -> None:
    ...
    assert "relationship_advice.json" in stages[2].outputs
    assert stages[-1].outputs == (
...
def test_document_classifier_detects_common_contract_inputs() -> None:
    assert (
        classify_document("Prime_Contract_Agreement.pdf") is DocumentType.PRIME_CONTRACT
    )
    assert (
        classify_document("General-Conditions-AIA-A201.pdf")
        is DocumentType.GENERAL_CONDITIONS
    )
    assert (
        classify_document("Insurance Requirements Exhibit.docx")
        is DocumentType.INSURANCE_REQUIREMENTS
    )
    assert classify_document("Board Resolution 2026.pdf") is DocumentType.BOARD_RECORD
    assert (
        classify_document("Proposed Capital Budget FY2027.pdf")
        is DocumentType.BUDGET_DOCUMENT
    )
...
def test_missing_required_documents_flags_bid_review_gaps() -> None:
    missing = missing_required_documents(
        [DocumentType.PRIME_CONTRACT, DocumentType.ADDENDUM]
    )
    assert missing == [
