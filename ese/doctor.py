...
def evaluate_doctor(cfg: Dict[str, Any]) -> Tuple[bool, List[str], Dict[str, str]]:
    ...
    if mode == "ensemble":
        ...
        provider_pairs, provider_pair_violations = _constraint_pairs(
            constraints,
            key="disallow_same_provider_pairs",
        )
...
def build_doctor_guidance(cfg: Dict[str, Any], violations: List[str]) -> List[str]:
    ...
    if any("share provider" in item for item in violations):
        guidance.append(
            "Separate constrained roles onto different providers or relax constraints.disallow_same_provider_pairs."
        )
