...
def _artifact_payloads(artifacts_dir: Path) -> dict[str, Any]:
    return {
        "decision_summary": read_json(artifacts_dir / "decision_summary.json"),
        "document_inventory": read_json(artifacts_dir / "document_inventory.json"),
        "risk_findings": read_json(artifacts_dir / "risk_findings.json"),
        "insurance_findings": read_json(artifacts_dir / "insurance_findings.json"),
        "compliance_findings": read_json(artifacts_dir / "compliance_findings.json"),
        "context_profile": read_json(artifacts_dir / "context_profile.json"),
        "procurement_profile": read_json(artifacts_dir / "procurement_profile.json"),
        "outcome_evidence": read_json(artifacts_dir / "outcome_evidence.json"),
        "obligations_register": read_json(artifacts_dir / "obligations_register.json"),
        "review_challenges": read_json(artifacts_dir / "review_challenges.json"),
    }


def _assert_contains(
    actual: set[str], expected: list[str], label: str, failures: list[str]
) -> None:
...
def evaluate_corpus_case(
    case_dir: str | Path, artifacts_root: str | Path | None = None
) -> CorpusEvaluationResult:
    case_path = Path(case_dir).expanduser().resolve()
    expected = read_json(case_path / "expected.json")
    ...
    compliance_categories = {
        item["category"] for item in payloads["compliance_findings"]
    }
    ...
    if (
        inventory["missing_required_documents"]
        != expected["missing_required_documents"]
    ):
        failures.append(
    ...
    _assert_contains(
        ...
        expected.get("insurance_categories", []),
        "insurance_categories",
        failures,
    )
    _assert_contains(
        compliance_categories,
        expected.get("compliance_categories", []),
        "compliance_categories",
        failures,
    )
    _assert_contains(
        obligation_titles,
        expected.get("obligation_titles", []),
        "obligation_titles",
        failures,
    )
    ...
    for key, value in expected_context.items():
        if context_profile.get(key) != value:
            failures.append(
                f"context_profile mismatch for {key}: expected {value}, got {context_profile.get(key)}"
            )
    ...
    for key, value in expected_outcome.items():
        if outcome_evidence.get(key) != value:
            failures.append(
                f"outcome_evidence mismatch for {key}: expected {value}, got {outcome_evidence.get(key)}"
            )
...
def evaluate_corpus(
    corpus_dir: str | Path, artifacts_root: str | Path | None = None
) -> list[CorpusEvaluationResult]:
